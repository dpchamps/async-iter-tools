...
describe.skip("pushToPull Async Iterator", () => {
  ...
  const getEventEmitterIterator = (ee: EventEmitter) =>
    pushToPull<EventEmitterIterator>((iterator) => {
      ee.on("exit", (statusCode: number) => {
        if (statusCode !== 0)
          return iterator.throw(`Received a non-zero statusCode`);
  ...
  it("Should handle some non-obvious racy things", async () => {
    ...
    await expect(asyncIter.next()).resolves.toEqual({
      done: false,
      value: "one",
    });
    await expect(asyncIter.next()).resolves.toEqual({
